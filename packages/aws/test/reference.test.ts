...
describe('persistent consumer reference', () => {
  ...
  it('scopes the reference identities and deploys before publication', async () => {
    const bootstrap = await readFile(join(repositoryRoot, 'infra/reference-bootstrap.yml'), 'utf8')
    expect(bootstrap).toContain('BucketName: ze-great-dashboard-reference-artifacts')
    expect(bootstrap).toContain('VersioningConfiguration: { Status: Enabled }')
    expect(bootstrap).toContain('SSEAlgorithm: AES256')
    expect(bootstrap).toContain('ref:refs/heads/main')
    expect(bootstrap).toContain('stack/ze-great-dashboard-reference/*')
    expect(bootstrap).toContain('function:ze-great-dashboard-reference')
    expect(bootstrap).toContain('role/ze-great-dashboard-reference-*')
    ...
    expect(workflow).toContain(`s3://ze-great-dashboard-reference-artifacts/\${artifact_key}`)
  })
})
